...
class AI_Assistant {
    ...
    private function calculateLocationScore($vendor, $event) {
        // Use event_lat/lng if available from event details
        $eventLat = $event['event_lat'] ?? null;
        $eventLng = $event['event_lng'] ?? null;

        if (!isset($vendor['business_lat']) || !isset($vendor['business_lng']) || !isset($eventLat) || !isset($eventLng)) {
            return 0.5; // Default score if location data missing for either vendor or event
        }
